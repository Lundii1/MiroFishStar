...
// Directly get maxRounds from query parameters during initialization to ensure child components can get the value immediately
...
const handleGoBack = async () => {
  // Before returning to Step 2, close the running simulation first
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close simulation environment, attempting forced stop...`)
        try {
          await stopSimulation({ simulation_id: currentSimulationId.value })
          addLog('✓ Simulation forced stopped')
        } catch (stopErr) {
...
const handleNextStep = () => {
  // Step3Simulation component handles report generation and route jumps directly
  // This method is only for backup
...
const loadGraph = async (graphId) => {
  // When simulating, auto-refresh does not show full-screen loading to avoid flickering
  // Show loading during manual refresh or initial load
  if (!isSimulating.value) {
...
const startGraphRefresh = () => {
  ...
  addLog('Graph real-time refresh started (30s)')
...
onMounted(() => {
  ...
  // Record maxRounds config (value obtained from query parameters during initialization)
  if (maxRounds.value) {
