...
const handleNextStep = (params = {}) => {
  ...
  // If there are custom rounds, pass through query params
  if (params.maxRounds) {
 ...
 * When the user returns to Step 2 from Step 3, the user defaults to exiting the simulation
 */
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Simulation environment closure exception: ${closeErr.message}`)
        // 如果优雅关闭异常，尝试强制停止
        await forceStopSimulation()
      }
    } else {
      // Environment not running, but process might still be there, check simulation status
    ...
    // Failure to check environment status does not affect subsequent processes
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(`Failed to forced stop simulation: ${stopRes.error || 'Unknown error'}`)
    }
  } catch (err) {
    addLog(`Forced stop simulation exception: ${err.message}`)
  }
}
